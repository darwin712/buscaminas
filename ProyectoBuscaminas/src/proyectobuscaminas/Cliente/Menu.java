...
public class Menu extends Fondo { 
    ...
    private void btnMusicActionPerformed(java.awt.event.ActionEvent evt) {                                         
        // Reproducir efecto de sonido (si existe y es compatible)
        ...
        if (Musica.getInstance().isPlaying()) {
            ...
            // Verificar si realmente prendió (por si el archivo falla)
            if (Musica.getInstance().isPlaying()) {
                btnMusic.setText("ON");
            } else {
                // Si falló (ej. formato no soportado), volver a OFF visualmente
                btnMusic.setText("OFF");
                // Opcional: Descomentar para debug
                // JOptionPane.showMessageDialog(this, "No se pudo reproducir la música.\nVerifica el archivo .ogg");
            }
        }
    }  
